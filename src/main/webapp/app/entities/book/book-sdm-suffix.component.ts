...
@Component({
    selector: 'jhi-book-sdm-suffix',
    templateUrl: './book-sdm-suffix.component.html'
})
export class BookSdmSuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/book-sdm-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/book-sdm-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BookSdmSuffix) {
        return item.id;
    }
